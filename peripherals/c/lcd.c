  ...
  for (i=0;i< ROWS ;i++)
  {
        for(j= 0; j < COLS; j++)
        {
            if( (j %8) == 0)
            {
              ...
              data = field[byte_index];
            }
            if ( data & 0x80)
            {
                lcd_write_data_u16(LCD_COLOR_WHITE);
            }
            else
            {
                lcd_write_data_u16(LCD_COLOR_GREEN);
            }
            data = data << 1;
        }
  }
...
{
	uint16_t lineColor = LCD_COLOR_WHITE;
	uint16_t fieldColor = LCD_COLOR_GREEN;
  uint16_t i,j;
  uint8_t data;
  uint16_t byte_index;
  uint16_t bytes_per_row;
  ...
  for (i=0;i< image_height_pixels ;i++)
  {
        for(j= 0; j < image_width_bits; j++)
        {
            ...
            if ( data & 0x80)
            {
                lcd_write_data_u16(iColor);
            }
						else if(get_data_background(x_start, y_start, field)){
								lcd_write_data_u16(lineColor);
						}
            else
            {
                lcd_write_data_u16(fieldColor);
